...
export function visible(s:State,role:Role){return {...s,entries:s.entries.filter(e=>allowed(e,role)),processes:s.processes.filter(p=>allowed(p,role)),contributions:s.contributions.filter(c=>role==='owner'||c.author===role),runs:s.runs.filter(r=>r.role===role),ledger:s.ledger.filter(l=>role==='owner'||l.role===role),rewards:s.rewards.filter(r=>role==='owner'||r.role===role),checks:role==='owner'?s.checks:[],handover:role==='owner'?s.handover:null,settings:{hasKey:!!s.settings.keyCipher,calls:s.settings.calls}};}
